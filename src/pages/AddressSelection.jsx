...
const AddressSelection = () => {
  ...
  useEffect(() => {
    ...
    const verificationStatus = localStorage.getItem("otpVerified");

    if (tempOrderData && verificationStatus === "true") {
      ...
      // If there's temp order data but no verification, clear it
      localStorage.removeItem("tempOrderData");
      toast.error("Phone verification is required to place the order.");
    }
  }, []);
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      // For COD, require OTP verification
      if (paymentMethod === "cod") {
        navigate("/otp", {
          state: {
            redirectTo: "/select-address",
  ...
  const processOrder = async (orderData, selectedAddress, paymentMethod) => {
    ...
    try {
      ...
      console.log("Formatted Order Data:", newOrderData);

      // Helper function to send confirmation email
      const sendOrderConfirmationEmail = async (orderDetails, paymentType) => {
        try {
          ...
          const emailResponse = await fetch(
            "https://qdore-backend-final-final-last.vercel.app/api/send-email/confirmationemail",
            {
              ...
              body: JSON.stringify({
                ...
                amount: orderDetails.amount || orderData.totalAmount, // Fallback to original amount
          ...
          if (!emailResponse.ok) {
            ...
            throw new Error(
              `Failed to send confirmation email: ${errorData.message}`
            );
          }
          ...
          // Don't throw error to prevent order process interruption
          toast.warning(
      ...
      if (paymentMethod === "cod") {
        ...
        // Send confirmation email for COD using the helper function
        await sendOrderConfirmationEmail(savedOrderData, "Cash on Delivery");
        ...
        const options = {
          ...
          handler: async function (response) {
            try {
              ...
              const savedOrderData = await orderResponse.json();
              console.log("Saved Online Order Data:", savedOrderData);
              ...
              // Send confirmation email for online payment using the helper function
